...
inference_funs = list(
  ...
  MQ_noconstr = function(x) estimateFPLD(x, big_support = FALSE))

score_funs = rep(list(function(y, par) mean(crps_fpld(y, par))), 4)
names(score_funs) = c("MQ", "ML", "starship", "MQ_noconstr")
...
inference_funs = list(
  ...
  MQ_noconstr = function(x) estimateFPLD(x, big_support = FALSE, positive_support = TRUE))
